...
def main():
    ...
    for digit in cifar10_traindata:
        split_cifar10_traindata.append(data_split(digit, 20))

    cifar10_testdata = []
    for number in range(10):
        idx = test_cifar10.target == number
        cifar10_testdata.append(test_cifar10.data[idx])
    split_cifar10_testdata = []
    for digit in cifar10_testdata:
        split_cifar10_testdata.append(data_split(digit, 20))
    ...
    digit_count = np.array([len(v) for v in split_cifar10_traindata])
    print('>>> Each digit in train data is split into: {}'.format(digit_count.tolist()))

    digit_count = np.array([len(v) for v in split_cifar10_testdata])
    ...
    for user in range(NUM_USER):
        ...
        for i, d in enumerate(choose_two_digit(split_cifar10_traindata)):
            if i:
                min_class_len = round(len(split_cifar10_traindata[d][-1]) * IMBA_RATIO)
                split_cifar10_traindata[d][-1] = split_cifar10_traindata[d][-1][:min_class_len]
                min_class_len = round(len(split_cifar10_testdata[d][-1]) * IMBA_RATIO)
                split_cifar10_testdata[d][-1] = split_cifar10_testdata[d][-1][:min_class_len]
